...
<div>
    ...
    <x-skeleton-rows wire:loading.delay wire:target="search,filter" :rows="max($rows->count(), 3)" class="mt-2" />

    <div wire:loading.delay.remove wire:target="search,filter">
        @if ($rows->isNotEmpty())
        ...
        @else
            <x-empty-state :title="$empty['title']">
                {{ $empty['message'] }}
                @if ($empty['add'])
                    <x-slot:action>
